      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="text-end my-2 pt-2">
            <!-- <button class="btn btn-primary btn-sm" @click="showAddEmployee = true">Cadastrar Funcionário</button> -->
...
import Swal from 'sweetalert2' 
  ...
  async function loadCompany(id) {
    try {
      ...
      employees.value = allEmployees.filter(e => String(e.companyId) === String(id))
    } catch (err) {
      console.error(err)
    }
  }
  ...
  async function loadEmployee() {
    try {
      const allEmployees = await EmployeeService.getEmployees()
      employees.value = allEmployees.filter(e => String(e.companyId) === String(props.companyId))
    } catch (err) {
      console.error(err)
    }
  }
